...
import (
	...
	"github.com/CogitoNTNU/cogi-go/internal/db"
...
func main() {
  ...
  logger := logrus.New().WithField("app", cfg.AppName).WithContext(ctx).WithField("router", r)

  logger.Info("Starting server")

  env := env.Configure(".env")

  db, err := db.InitDb(env, logger, ctx)
  if err != nil {
